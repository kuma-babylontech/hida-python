...
```python
CURRENT_YEAR = datetime.now().year  # 今年の西暦を自動で取得
...
4. `CURRENT_YEAR - 1995` で築年数が出る（年をまたいでも自動で正しい値）
...
- 点を「縦＝価格」「横＝築年数」の位置に打つだけ
- 点の散らばり方で **2つの値の関係** がわかる

**どんな時に使う？**

- 「Aが増えるとBも増える？減る？」を知りたい時
- 例: 築年数と価格／駅距離と価格／面積と価格
...
| 見え方 | 意味 |
|---|---|
| 右下がりに並ぶ | 一方が増えるともう一方は **減る** |
| 右上がりに並ぶ | 一方が増えるともう一方も **増える** |
| バラバラ | **関係がうすい** |
...
- 1つのグループの **「真ん中と広がり」** を箱で描く
- 複数のグループを横に並べると **一瞬で比較** できる
...
- すべての列どうしの「仲の良さ」を **表にまとめたもの**
- 色の濃さ（ヒートマップ）で **強さを可視化**

**どんな時に使う？**

- 「価格に効きそうな要因はどれ？」を最初に探したい時
- 列が多いデータで **当たり** を付ける
...
| 値 | 意味 |
|---|---|
| **+1 に近い** | 一緒に増える（例: 面積と価格） |
| **−1 に近い** | 片方が増えると片方は減る（例: 築年数と価格） |
| **0 に近い** | **関係がうすい** |
...
- `.corr()` = **全部の列の相関係数を一気に計算**
- `sns.heatmap` = 数字を色付きの表にしてくれる
- `annot=True` で各マスに数字も表示
...
- 対角線は常に **1**（自分と自分の関係なので当然）
- **色が濃い = 関係が強い**
- 築年数 vs 価格 = **−0.42**（築年数が増えると価格は下がる）

→ 「どの要因を深掘りすべきか」の **地図** になる
