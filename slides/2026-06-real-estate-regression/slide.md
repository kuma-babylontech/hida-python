...
date: 2026-06-14
description: "statsmodelsを使って単回帰・重回帰分析を学び、「築年数が1年増えると価格はいくら下がる？」「駅徒歩1分の価値は？」を数値化します。"
tags:
  - データ分析
  - 統計
  - 回帰分析
...
```
                 coef    std err      t      P>|t|
---------------------------------------------------
const        1837.4      56.4    32.6      0.000
BuildingAge   -23.4       2.2   -10.5      0.000
---------------------------------------------------
R-squared:  0.181
```
...
```
BuildingAge の coef = -23.4
```

→ **築年数が 1年増えるごとに、価格は約 23 万円下がる**

```
const の coef = 1837.4
```

→ 築年数 0年（新築）なら **約 1,837 万円** が出発点

```
R-squared = 0.181
```

→ 価格のバラつきのうち **18.1% を築年数だけで説明できる**
→ 残り 81.9% は他の要因（面積・駅距離など）
...
【重回帰】 価格 = a × 築年数 + b × 駅徒歩 + c × 面積 + d
```

- **「重」** = 原因を **複数** 同時に考える
- 現実の価格は 1つの要因だけで決まらない
- 「築年数 **も** 駅距離 **も** 面積 **も** 考慮した価格」がわかる
...
```
                       coef   std err      t      P>|t|
--------------------------------------------------------
const                 801.8    131.8     6.1      0.000
BuildingAge           -21.8      2.0   -11.1      0.000
TimeToNearestStation  -27.4      4.8    -5.7      0.000
Area                   52.8      5.0    10.5      0.000
--------------------------------------------------------
R-squared:  0.368
```
...
| 要因 | 係数（coef） | 意味 |
|---|---|---|
| BuildingAge | −21.8 | 築 1年で **約 22万円 下がる** |
| TimeToNearestStation | −27.4 | 駅徒歩 1分で **約 27万円 下がる** |
| Area | +52.8 | 1㎡ 広いと **約 53万円 上がる** |

**R² = 0.368** → 約 37% を説明できる（単回帰の 18% から大幅アップ）

→ **駅徒歩 1分の価値は約 27万円**（これが前回予告した答え）
...
| | 単回帰 | 重回帰 |
|---|---|---|
| 使った要因 | 築年数だけ | 築年数 + 駅距離 + 面積 |
| R²（説明力） | 0.181 | **0.368** |
| 築年数の係数 | −23.4 | −21.8 |

- 重回帰で R² が **倍以上** に上がった
- 築年数の係数が −23.4 → −21.8 に変化
  → 単回帰では「駅距離や面積の影響」も築年数に混ざっていた
...
--------------------------------------------------------
const                 801.8      0.000  ✓
BuildingAge           -21.8      0.000  ✓
TimeToNearestStation  -27.4      0.000  ✓
Area                   52.8      0.000  ✓
```

3つの要因すべてが **p < 0.05** → すべて **統計的に有意**

→ 「築年数・駅距離・面積はどれも本当に価格に影響している」
　 と **自信を持って** 言える
...
```python
# 例えば「取引の四半期」を追加してみると…
X = df[["BuildingAge", "TimeToNearestStation",
        "Area", "Quarter"]]
```
...
```
相場価格 = -21.8 × 築年数 - 27.4 × 駅徒歩 + 52.8 × 面積 + 801.8
```
...
|---|---|
| 単回帰 | 築年数だけで価格の傾向をつかむ |
| 重回帰 | 築年数 + 駅距離 + 面積で精度アップ |
| 係数（coef） | 「1年で何万円」「1分で何万円」 |
| p値 | その要因が本当に効いてるかの証拠 |
| 残差 | 相場と実際の差 → 割安/割高の判断 |
