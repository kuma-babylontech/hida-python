...
|---|---|
| Part 1（モックデータ） | −0.42 |
| **今回（実データ2024・約7,000件）** | **−0.55** |

- どちらも **中程度の負の相関**（築年数↑ → 価格↓）
  - 目安: 相関の絶対値 0.4〜0.7 = そこそこ関係あり／0.7以上 = 強い
- 実データの方が **やや強い**（モックより築年数がよく効く）
- **r² = 0.30** → 築年数だけで価格の **約30%** を説明（残り70%は他の要因）
- ちなみに **面積 × 価格 の相関も +0.55** → だから次の重回帰で精度が伸びる
...
| 前回（モック・500件） | 実データ（本番API・約7,000件） |
|---|---|
| 最寄駅・駅徒歩あり（自作） | **駅の項目が無い！** |
| 中央値 1,169 万円 | 中央値 **約 2,300 万円** |
| データはいつもキレイ | `１Ｋ` なのに 135㎡ 等の **誤登録あり** |

→ 予告した「駅徒歩1分の価値」は **出せなくなった**ので、
　 実在する **面積・築年数** で分析を組み直します
　（面積は **10〜40㎡** に絞ってワンルームの誤登録を除外）
...
- `y = ax + b` は **直線（一次関数）** の式そのもの
  - 横軸 `x` = 築年数、縦軸 `y` = 価格 に置き換えただけ
- だから散布図に **まっすぐな線** を引く＝この式を決めること

→ 新しい数式ではなく、**見慣れた直線の式に名前を付けただけ**

--

### 「必ずこの式」ではない

- **単回帰** = 説明変数が **1つ** という意味（築年数だけ）
- それを **直線で** 表すと → 必ず `y = ax + b`（＝今回やる線形単回帰）
- 曲線で当てることも本当はできる:

```
価格 = a × 築年数 ²  + b × 築年数 + c   （曲線の例）
```

- でも前回の散布図が **だいたいまっすぐ** だったので、
  まずは **いちばん単純な直線** から始める
...
```
                 coef    std err      t      P>|t|
---------------------------------------------------
const        3363.5      21.6   155.9      0.000
BuildingAge   -44.3       0.8   -55.0      0.000
---------------------------------------------------
R-squared:  0.300
```
...
```
BuildingAge の coef = -44.3
```

→ **築年数が 1年増えるごとに、価格は約 44 万円下がる**

```
const の coef = 3363.5
```

→ 築年数 0年（新築）なら **約 3,364 万円** が出発点

```
R-squared = 0.300
```

→ 価格のバラつきのうち **30.0% を築年数だけで説明できる**
→ 残り 70.0% は他の要因（面積など）
...
> 価格のちがい（ばらつき）のうち、
> 築年数で説明できるのは **たった 3割（30%）**

- 築年数でわかるのは **3割だけ**
- 残りの **7割** は、築年数 **以外** で決まっている
...
築年数で説明できない 7割は、2種類に分けられます。

| 種類 | 例 | どうする？ |
|---|---|---|
| **測れる他の要因** | 面積・エリア・階数 | **変数を足せば説明できる** |
| **偶然・個別の事情** | 売り急ぎで安く売った等 | 消せない（ノイズ） |

- だから R² が 100% になることは、まず無い
- でも「測れる要因」を足せば、散らばりは **減らせる**

→ それが次の **重回帰**（築年数 **＋ 面積**）。
　 R² は **0.30 → 0.49** に上がります
...
```
               coef   std err      t      P>|t|
------------------------------------------------
const        1001.1     49.9    20.0      0.000
Area           96.5      1.9    50.9      0.000
BuildingAge   -36.2      0.7   -51.1      0.000
------------------------------------------------
R-squared:  0.488
```
...
| 要因 | 係数（coef） | 意味 |
|---|---|---|
| Area | +96.5 | 1㎡ 広いと **約 96万円 上がる** |
| BuildingAge | −36.2 | 築 1年で **約 36万円 下がる** |

**R² = 0.488** → 約 49% を説明できる（単回帰の 30% から大幅アップ）

→ **面積 1㎡ の価値は約 96万円**（ワンルームでは面積が価格を大きく左右する）
...
- 重回帰で R² が **大きく** 上がった（0.30 → 0.49）
- 築年数の係数が −44.3 → −36.2 に変化
...
- p値 = 「本当は **関係ない** のに、これ以上に極端な結果が出る確率」
...
- 0.05 = 5% → 「偶然これ以上に極端な差が出る確率が 5% 未満」
- 0.000 と表示されたら「ほぼ 0%」= **偶然とは考えにくい**
...
const        1001.1      0.000  ✓
Area           96.5      0.000  ✓
BuildingAge   -36.2      0.000  ✓
```

2つの要因どちらも **p < 0.05** → すべて **統計的に有意**

→ 「面積も築年数も、偶然ではなく価格に関係していそう」と言える
　 （※ あくまで **相関**。「築年数のせいで下がる」と因果まで断定はできない）

--

### もしp値が大きい要因があったら？

**取引の四半期** = その物件が **いつ売れたか**（`Period` から作る 1〜4 の数字。
第1＝1〜3月／第2＝4〜6月／第3＝7〜9月／第4＝10〜12月）

```python
# 「いつ売れたか」も価格に効くか試す（Period から四半期を作る）
df["Quarter"] = df["Period"].str.extract(r"第(\d)四半期").astype(float)
X = df[["Area", "BuildingAge", "Quarter"]]
```

```
               coef      P>|t|
Quarter        12.0      0.153   ← 0.05 より大きい！
```

→ **p値 0.153** = 「偶然こうなる確率が 約15%」
→ この要因は **効いてない可能性が高い** → モデルから外す
　（実際 R² も **0.488 のまま**＝足しても説明力は増えない）
...
東京・2024年の中古ワンルーム **約7,000件** で計算すると…

| 割安の例 | 面積 | 築年 | 実価格 | 相場 | 差（残差） |
|---|---|---|---|---|---|
| 八王子市 | 30㎡ | 7年 | 1,900万 | 3,643万 | **−1,743万** |
| 足立区 | 30㎡ | 10年 | 1,800万 | 3,534万 | **−1,734万** |

- 相場より **500万円以上安い** 物件が **約21%**（約1,500件）もある
...
- このモデルは **面積と築年数だけ** で相場を計算している
- **エリア（立地）を見ていない** → 郊外なのに都心並みの相場を予測
- だから「割安」＝本当にお得とは限らない（**モデルの見落とし**かも）

→ 残差は **「調べるべき候補」を出す道具**。
　 最後は人が中身を確認する（次回の機械学習でエリアも考慮）
...
全物件の **残差（相場とのズレ）** を集めて、
「どんなズレが何件あるか」を棒グラフにしたもの

- **目的**: 個々の物件ではなく、**モデル全体がまともか**を確認する
- **見るポイント**:
  - 山が **0 の周り** に集まる → 相場をうまく当てている
  - 大きく **左右どちらかに偏る** → モデルがどこか間違っている
...
plt.axvline(0, color="red", linestyle="--")  # 残差0の線
plt.xlabel("残差（万円）")
```
...
- 横軸 = 残差（相場とのズレ・万円）／縦軸 = 物件の件数
- 山が **0 のあたりに集中** → 多くの物件は相場どおり＝**モデルは健全**
- **0 より左 = 割安**（相場より安い）／右 = 割高
- ズレの大きさはだいたい **±800万円**（標準偏差 約785万）

→ 左のすそが割安候補。ただし極端なものは特殊事情・エリア未考慮かも
