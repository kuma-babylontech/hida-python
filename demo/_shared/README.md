...
- 環境変数 `REINFOLIB_API_KEY` が **あれば本番 API**、なければ本番 API から取得済みの
  **実データスナップショット**（`tokyo_oneroom_2024.csv`／東京・2024年通年・中古ワンルーム）で
  代替する。どちらも同じ正規化を通すため、鍵の有無で結果は変わらない。
- ワンルーム抽出は間取り（1R/1K）に加えて**面積 10〜40㎡**でも絞る。実データには間取り
  「１Ｋ」のまま 135㎡ といったワンルームでない誤登録が混じるため。
- 本番取得を試す:
...
（公式マニュアルの出力項目に駅情報なし）。**駅徒歩を使った分析は実データではできない**ため、
...
| `tokyo_oneroom_2024.csv` | 本番 API から取得した実データスナップショット（東京・2024通年・中古ワンルーム）。鍵が無いときの既定データ |
| `tests/` | 和暦変換・全角処理・型変換・API 呼び出し・面積帯フィルタのテスト |

> スナップショットは国土交通省「不動産情報ライブラリ」の公開オープンデータ。最新化したい
> 場合は鍵を設定して `reinfolib.fetch_transactions` で再取得し、CSV を差し替える。
