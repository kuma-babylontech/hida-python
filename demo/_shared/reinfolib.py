...
環境変数 ``REINFOLIB_API_KEY`` があれば本番 API を叩き、無ければ本番 API から取得済みの
実データスナップショット（``tokyo_oneroom_2024.csv``）で代替する。どちらも同じ正規化を
通すので、鍵の有無で結果は変わらない。
"""
...
#: 鍵が無いときに使うオフラインデータ。本番 API から取得した実データのスナップショット
#: （東京・2024年通年・中古ワンルーム）。詳細は README を参照。
OFFLINE_CSV = Path(__file__).resolve().parent / "tokyo_oneroom_2024.csv"

#: ワンルーム/1K として扱う専有面積の範囲（㎡）。実データには間取り "１Ｋ" のまま
#: 135㎡ といったワンルームでない誤登録が混じるため、面積でも妥当性を担保する。
ONEROOM_AREA_MIN = 10
ONEROOM_AREA_MAX = 40
...
def _load_offline_records() -> list[dict]:
    """オフラインの実データスナップショットを「全項目文字列」の dict リストで返す。"""
    return pd.read_csv(OFFLINE_CSV, dtype=str).fillna("").to_dict("records")
...
def load_dataframe(
    ...
    *,
    ...
    鍵があれば本番 API（指定した年・四半期を全取得）、無ければオフラインの実データ
    スナップショット（東京・2024年通年）。正規化したうえで、ワンルーム・1K に絞り込み、
    分析に必須の列が欠損する行を落とす。

    既定の年が 2024 なのは、同梱スナップショットと一致させ、鍵あり／無しで同じ結果に
    なるようにするため。
    """
    if is_live():
        frames = [
        ...
        raw = pd.DataFrame(_load_offline_records())
    ...
    if oneroom_only and "FloorPlan" in df.columns:
        # 間取りが 1R/1K かつ、面積もワンルーム相当（誤登録の大面積を除外）。
        mask = df["FloorPlan"].astype(str).str.contains("１Ｒ|１Ｋ|1R|1K", na=False)
        if "Area" in df.columns:
            mask &= df["Area"].between(ONEROOM_AREA_MIN, ONEROOM_AREA_MAX)
        df = df[mask]
...
def main() -> None:
    ...
    mode = "本番 API" if is_live() else "オフライン実データスナップショット（2024）"
    print(f"データソース: {mode}")
